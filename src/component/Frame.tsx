...
import {OptionBoxContainer, AddItemInputContainer} from '../lib/Styled'
...
const AddItemInput = (props: AddItemInputProps) => {
    ...
    const doAdd = (e: React.FormEvent<HTMLFormElement>) => {
        ...
        props.setList(prev => prev === undefined ? [value1] : [...prev.concat(value1)])
        setValue1(prev => "")
        inputRef.current?.focus()
    }
    // 항목 수정 내용
            ...
            <ul>
                {temp?.map((item, idx) => (
                    <form key={`${idx}`} onSubmit={handleItem} data-idx={`${idx}`} >
                        <input value={item} onChange={onChange2} onFocus={()=>{!props.readonly && setInputIdx(prev => idx)}} readOnly={props.readonly || false}/>
                        <button type={"submit"}>{idx === inputIdx ? "수정" : "삭제"}</button>
                    </form>
                ))}
            </ul>
        </AddItemInputContainer>
